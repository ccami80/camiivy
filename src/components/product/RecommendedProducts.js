...
export default function RecommendedProducts({ productId }) {
  ...
  return (
    <section className="mt-20 border-t border-gray-200 pt-10 md:mt-24 md:pt-12" aria-label="함께 구매하면 좋은 상품">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900">
          함께 구매하면 좋은 상품이에요
          <span className="ml-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-gray-200 text-xs text-gray-500" aria-hidden>?</span>
        </h2>
        {list.length > 0 && (
      ...
      {loading ? (
        ...
        <p className="mt-4 text-sm text-gray-500">추천 상품이 없습니다.</p>
      ) : (
